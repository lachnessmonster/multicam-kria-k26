...
**Target modes:** 1280x720 for bring-up, then 1920x1080 for the useful
wide-angle field of view. The current `failed to read chip id ... -5`
must be cleared before either mode can be tested (see Camera enable).
...
| required i2c address | sensor at 0x1a | sensor at 0x1a |
...
Arducam's Raspberry Pi configuration for the manual-lens SKU disables VCM
control. Deleting the `ak7375@c` node from this overlay is the KV260
equivalent. Do not infer the fitted lens from an I2C scan: auxiliary
addresses such as 0x0c and 0x58 can still exist on a module PCB. The only
address required for image capture is the IMX519 at 0x1a.
...
```bash
make rebuild-and-deploy  # Vivado 2026.1: force rebuild, enforce F11, deploy
make deploy-kmod
```

For later software-only changes, `make deploy` reuses the checked XSA in
`build-hw/`; if it does not exist, it builds it first. The archived image in
`hardware/` is retained only for comparison and is never the default.

```bash
make bitstream-id    # md5 of repo vs board -- run this first, always
```

### Camera enable: the fix for chip-ID `-EIO`

`design_1.tcl` creates the `rpi_cam_en` port. The corrected script now
imports `constrs_1.xdc` itself, and `hardware/build-hardware.tcl` refuses
to export unless implementation places the port on F11 with the KV260
reference electrical settings. Earlier projects could be regenerated
without that XDC and produce a bitstream that loaded normally with camera
enable floating.

The failure is nasty because it does not look like a bitstream problem.
`rpi_cam_en` gates the sensor's 24 MHz INCK, and an IMX519 without INCK
will not ACK its own i2c address — while the other devices on the module,
which need nothing but 3.3 V, answer normally. So the bus scans clean,
the mux is fine, the driver binds, and it dies at the chip-id read:

```
imx519 6-001a: failed to read chip id 519, with error -5
```

Every instinct says driver or device tree. In this failure state it is the
PL image, and no overlay edit can reach it: `rpi_cam_en` is tied to an
`xlconstant` in the fabric, so there is no GPIO or `devmem` escape hatch.
If you see `-EIO` on the chip ID with 0x1a absent from `i2cdetect` but
0x0c/0x58 present, run `make rebuild-and-deploy` before changing the driver.

The archived XSA predates the reproducible constraint import, and an XSA
does not expose enough implementation constraint data to prove its F11 LOC.
It remains available for comparison, but is no longer labelled “golden.”
