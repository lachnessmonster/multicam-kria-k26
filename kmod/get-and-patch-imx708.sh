#!/bin/bash
# Fetch raspberrypi/linux imx708.c and apply the minimum needed to build
...
# link-frequency that does not equal its own IMX708_DEFAULT_LINK_FREQ,
...
awk '
  ...
  { print }
' imx708.c.orig > imx708.c

echo "patched imx708.c ready ($(wc -l < imx708.c) lines)"
